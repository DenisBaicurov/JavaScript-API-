...
function itemRecords (item, signUp, cancelRecording) {
   return `
      <div class="itemTraining">
         ...
         <p class="maxcurrentParticipants">Максимальное количество мест:<span class="maxnumbercurrentParticipants"> ${item.maxParticipants}</span></p>
...
if(Number(maxnumbercurrentParticipants.item(0).textContent)<=numbercurrentParticipants.item(0).textContent)
 {item.disabled = true;}
...
if(Number(numbercurrentParticipants.item(0).textContent)<=0)
  { item.disabled = true;}
